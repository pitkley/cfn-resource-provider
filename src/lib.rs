...
///
/// When this library creates the response which will be sent to AWS CloudFormation, by default a
/// physical resource ID will be created according to the following format (where `suffix` will be
/// the suffix provided by the implementor):
///
/// ```text
...
///
/// ## Overwriting the physical resource ID
///
/// If you don't want to use the physical resource ID generated by the default implementation, you
/// can override the `physical_resource_id` function and return your own ID. This can be useful if
/// the physical resource ID of your custom resource does not depend on the stack ID or the logical
/// resource ID, but only on other factors.
///
/// ## Example
///
...
/// }
/// impl PhysicalResourceIdProvider for MyResourcePropertiesType {
...
pub trait PhysicalResourceIdProvider {
    /// Create a physical resource ID that uniquely identifies the resources represented by the type
    /// holding the AWS CloudFormation resource properties.
    ///
    /// The default implementation returns a custom ARN in the following format:
    ///
    /// ```text
    /// arn:custom:cfn-resource-provider:::{stack_id}-{logical_resource_id}/{suffix}
    /// ```
    ///
    /// The stack and logical resource IDs are taken out of the provided request, the suffix is
    /// generated by the [`physical_resource_id_suffix`] method.
    ///
    /// A trait implementor only has to implement the [`physical_resource_id_suffix`] method,
    /// although they are free to overwrite the entire ID if desired (in which case the suffix-
    /// method can be stubbed, if desired).
    ///
    /// [`physical_resource_id_suffix`]: #tymethod.physical_resource_id_suffix
    fn physical_resource_id(&self, request: &CfnRequest<Self>) -> String
    where
        Self: Clone,
    {
        match request {
            ...
            | CfnRequest::Update {
                ...
                let suffix = self.physical_resource_id_suffix();
                format!(
...
impl<T> PhysicalResourceIdProvider for Option<T>
where
    T: PhysicalResourceIdProvider,
{
    fn physical_resource_id_suffix(&self) -> String {
        match self {
...
impl PhysicalResourceIdProvider for () {
    fn physical_resource_id_suffix(&self) -> String {
        String::new()
    }
}
...
impl<P> CfnRequest<P>
where
    P: PhysicalResourceIdProvider + Clone,
{
    ...
    pub fn request_id(&self) -> String {
        match self {
            CfnRequest::Create { request_id, .. }
            | CfnRequest::Delete { request_id, .. }
    ...
    pub fn response_url(&self) -> String {
        match self {
            CfnRequest::Create { response_url, .. }
            | CfnRequest::Delete { response_url, .. }
    ...
    pub fn resource_type(&self) -> String {
        match self {
            CfnRequest::Create { resource_type, .. }
            | CfnRequest::Delete { resource_type, .. }
    ...
    pub fn stack_id(&self) -> String {
        match self {
            CfnRequest::Create { stack_id, .. }
            | CfnRequest::Delete { stack_id, .. }
    ...
    pub fn physical_resource_id(&self) -> String {
        self.resource_properties().physical_resource_id(self)
    }
...
impl PhysicalResourceIdProvider for Ignored {
    fn physical_resource_id_suffix(&self) -> String {
        String::new()
    }
}
...
///
/// This function will, in conjunction with the official [Rust AWS Lambda runtime], deserialize the
...
///
/// `process` expects a single parameter, which should be a function or closure that receives a
/// [`CfnRequest<P>`][CfnRequest] as its only parameter, and is expected to return a
/// `Future<Result<Option<S>, E>`.The type returned for success, `Option<S>`, where `S` needs to be
/// serializable. The error-type is expected to be something that implements/can be turned into a
/// [`std::error::Error`], which is true for most errors returned. ([anyhow] is a library that can
/// help with error-handling, and is something that returns the correct kind of error.)
///
/// The computation required to create your custom resource should happen in the provided
/// function/closure.
///
...
pub fn process<F, P, Fut, S, E>(f: F) -> Handler<F>
where
    F: Fn(CfnRequest<P>) -> Fut + 'static,
    P: PhysicalResourceIdProvider + Clone + Send + 'static,
    P: for<'de> Deserialize<'de>,
    Fut: Future<Output = Result<Option<S>, E>> + Send + 'static,
    S: Serialize + Send + 'static,
    E: Into<Error> + std::fmt::Display + std::fmt::Debug + 'static,
{
    Handler { f: Some(f) }
}

/// The type used to wrap the communication with the official [Rust AWS Lambda runtime], handling
/// the communication with CloudFormation after the user-provided code has acted on (or failed to
/// act on) the custom resource.
///
/// [Rust AWS Lambda runtime]: https://github.com/awslabs/aws-lambda-rust-runtime
...
impl<F, P, Fut, S, E> lambda::Handler<CfnRequest<P>, Option<S>> for Handler<F>
where
    F: Fn(CfnRequest<P>) -> Fut + 'static,
    P: PhysicalResourceIdProvider + Clone + Send + 'static,
    P: for<'de> Deserialize<'de>,
...
{
    type Error = Error;
    type Fut = Pin<Box<dyn Future<Output = Result<Option<S>, Error>>>>;

    fn call(&mut self, event: CfnRequest<P>, _context: lambda::Context) -> Self::Fut {
        let f = std::mem::take(&mut self.f);
        Box::pin(process_impl(event, f.expect("invalid application state")))
    }
}

async fn process_impl<F, P, Fut, S, E>(request: CfnRequest<P>, f: F) -> Result<Option<S>, Error>
where
    F: Fn(CfnRequest<P>) -> Fut,
    P: PhysicalResourceIdProvider + Clone + Send,
    P: for<'de> Deserialize<'de>,
    Fut: Future<Output = Result<Option<S>, E>> + Send,
    S: Serialize + Send,
    E: Into<Error> + std::fmt::Display + std::fmt::Debug + 'static,
{
    // Get result from provided closure
    let request_result = f(request.clone()).await;

    // Turn the request and the result into the response we'll send to CloudFormation.
    let cfn_response_url = request.response_url();
    let cfn_response = serde_json::to_string(&request.into_response(&request_result))?;

    // Send the result to CloudFormation, raising for any errors.
    let client = reqwest::Client::new();
    let response = client
        .put(&cfn_response_url)
        .header("Content-Type", "")
        .body(cfn_response)
        .send()
        .compat()
    ...
    // Return the result from the user to the user.
    request_result.map_err(Into::into)
}
...
mod test {
    ...
    #[derive(Debug, Clone)]
    struct Empty;
    impl PhysicalResourceIdProvider for Empty {
        fn physical_resource_id_suffix(&self) -> String {
            String::new()
        }
    }
    ...
    #[derive(Debug, Clone)]
    struct StaticSuffix;
    impl PhysicalResourceIdProvider for StaticSuffix {
        fn physical_resource_id_suffix(&self) -> String {
    ...
    impl PhysicalResourceIdProvider for ExampleProperties {
        fn physical_resource_id_suffix(&self) -> String {
    ...
    fn cfnresponse_from_cfnrequest_unit() {
        ...
        let actual_response =
            serde_json::to_value(actual_request.into_response(&Ok::<Option<()>, String>(None)))
                .unwrap();
        let expected_response = json!({
            "Status": "SUCCESS",
            "RequestId": "unique id for this create request",
    ...
    fn cfnresponse_from_cfnrequest_serializable() {
        ...
        let actual_response = serde_json::to_value(actual_request.into_response(&Ok::<_, String>(
